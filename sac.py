...
class SAC(object):
    def __init__(self, num_inputs, action_space, args):
        ...
        if self.policy_type == "Gaussian":
            ...
            if self.hyper:
                self.policy =  hyperActor(action_space.shape[0], num_inputs, action_space.high[0], np.arange(4,512 + 1), meta_batch_size = args.meta_batch_size, device=self.device, search = args.search).to(self.device)
    ...
    def load_checkpoint(self, ckpt_path, evaluate=False, add_search_params=False):
